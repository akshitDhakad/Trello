"use client";

import React, { useEffect, useRef, useState } from "react";
import { usePathname } from "next/navigation";
import Link from "next/link";
import Image from "next/image";
import SidebarItem from "@/components/Sidebar/SidebarItem";
import ClickOutside from "@/components/ClickOutside";
import useLocalStorage from "@/hooks/useLocalStorage";
import {
  CalendarIcon,
  ChatBubbleBottomCenterIcon,
  Squares2X2Icon,
  Cog6ToothIcon,
  TableCellsIcon,
  BellAlertIcon,
  ChartPieIcon,
  PlusIcon,
  PowerIcon,
} from "@heroicons/react/24/outline";
import { UserIcon } from "@heroicons/react/24/outline";
import { ListBulletIcon } from "@heroicons/react/24/outline";

interface SidebarProps {
  sidebarOpen: boolean;
  setSidebarOpen: (arg: boolean) => void;
}

const icons = {
  className: "h-4 lg:6 w-4 lg:6",
};

const menuGroups = [
  {
    name: "MENU",
    menuItems: [
      {
        icon: <Squares2X2Icon {...icons} />,
        label: "Dashboard",
        route: "/",
      },
      {
        icon: <CalendarIcon {...icons} />,
        label: "Calendar",
        route: "/calendar",
      },
      {
        icon: <ChatBubbleBottomCenterIcon {...icons} />,
        label: "Chat",
        route: "/chat",
      },
      {
        icon: <UserIcon {...icons} />,
        label: "Profile",
        route: "/profile",
      },
      {
        icon: <ListBulletIcon {...icons} />,
        label: "Forms",
        route: "#",
        // children: [
        //   { label: "Form Elements", route: "/forms/form-elements" },
        //   { label: "Form Layout", route: "/forms/form-layout" },
        // ],
      },
      {
        icon: <TableCellsIcon {...icons} />,
        label: "Tables",
        route: "/tables",
      },
      {
        icon: <Cog6ToothIcon {...icons} />,
        label: "Settings",
        route: "/settings",
      },
      {
        icon: <ChartPieIcon {...icons} />,
        label: "Chart",
        route: "/chart",
      },
      {
        icon: <BellAlertIcon {...icons} />,
        label: "Notification",
        route: "/ui/alerts",
      },
      {
        icon: <PlusIcon {...icons} />,
        label: "Add New",
        route: "/ui/alerts",
      },
    ],
  },
];

const Sidebar = ({ sidebarOpen, setSidebarOpen }: SidebarProps) => {
  const pathname = usePathname();
  const [pageName, setPageName] = useLocalStorage("selectedMenu", "dashboard");

  return (
    <ClickOutside onClick={() => setSidebarOpen(false)}>
      <aside
        className={`fixed left-0 top-0 z-9999 flex h-screen flex-col overflow-y-hidden bg-black duration-300 ease-linear dark:bg-boxdark lg:translate-x-0 ${
          sidebarOpen ? "translate-x-0" : "-translate-x-full"
        }`}
      >
        {/* <!-- SIDEBAR HEADER --> */}
        <div
          className={`${sidebarOpen ? "block" : "hidden"} flex items-center justify-between gap-2 px-4 py-2 lg:py-4`}
        >
          <Link href="/">
            <Image
              width={100}
              height={24}
              src={"/images/logo/logo.svg"}
              alt="Logo"
              priority
            />
          </Link>

          <button
            onClick={() => setSidebarOpen(!sidebarOpen)}
            aria-controls="sidebar"
            className="mx-1.5 block"
          >
            <svg
              className="fill-current"
              width="12"
              height="18"
              viewBox="0 0 20 18"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M19 8.175H2.98748L9.36248 1.6875C9.69998 1.35 9.69998 0.825 9.36248 0.4875C9.02498 0.15 8.49998 0.15 8.16248 0.4875L0.399976 8.3625C0.0624756 8.7 0.0624756 9.225 0.399976 9.5625L8.16248 17.4375C8.31248 17.5875 8.53748 17.7 8.76248 17.7C8.98748 17.7 9.17498 17.625 9.36248 17.475C9.69998 17.1375 9.69998 16.6125 9.36248 16.275L3.02498 9.8625H19C19.45 9.8625 19.825 9.4875 19.825 9.0375C19.825 8.55 19.45 8.175 19 8.175Z"
                fill=""
              />
            </svg>
          </button>
        </div>
        {/* <!-- SIDEBAR HEADER --> */}
        <div className="no-scrollbar flex flex-col overflow-y-auto duration-300 ease-linear">
          {/* <!-- Sidebar Menu --> */}
          <nav className="mt-4 px-4 py-4 lg:mt-9 lg:px-4">
            {menuGroups.map((group, groupIndex) => (
              <div key={groupIndex}>
                <h4 className="xl:lg-4 mb-4 text-center text-xs font-semibold lg:text-start">
                  {group.name}
                </h4>

                <ul className="mb-6 flex flex-col gap-1.5">
                  {group.menuItems.map((menuItem, menuIndex) => (
                    <SidebarItem
                      sidebarOpen={sidebarOpen}
                      setSidebarOpen={setSidebarOpen}
                      key={menuIndex}
                      item={menuItem}
                      pageName={pageName}
                      setPageName={setPageName}
                    />
                  ))}
                </ul>
              </div>
            ))}
          </nav>
          {/* <!-- Sidebar Menu --> */}
        </div>
        <div className="flex items-center justify-center py-4 lg:py-6">
          <PowerIcon {...icons} />
        </div>
      </aside>
    </ClickOutside>
  );
};

export default Sidebar;
